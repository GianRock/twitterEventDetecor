...
object SpMongo {
  ...
  def getTweetsAsTupleRDD(sc:SparkContext,query:Document,  createSplits:Boolean=true):RDD[(Long, Tweet)]={
    ...
    val mongoUri=if(auth){
      "mongodb://"+usr+":"+"sparkmongo"+"@"+host+":27017/"+ tweetdb+".tweets?authSource="+authdb
    }else{
      "mongodb://"+host+":27017/"+ tweetdb+".tweets"
    }
...
val mindate=TweetCollection.findMinMaxValueDate()
    val endDate=new Date(mindate.getTime+(3600000*10))
    val tweets: RDD[(Long, Tweet)] =getTweetsAsTupleRDD(sc,null,false)
      //getTweetsAsRDDInTimeInterval(sc,mindate,endDate)
